...
| 停止跟随 | "别跟了"、"停止跟随"、"不要跟了"、"停止手势" | 停止当前跟随模式，释放 TCM 资源 |
...
配置文件默认提供 4 种本地模型选择，通过配置文件、命令行方式传入均可，不传则使用默认配置（`qwen2.5:1.5b`）
...
### 4.4 大模型自主编排动作 (LLM Motion Choreography)

除了固定的关键词匹配动作外，系统已集成基于 MCP (Model Context Protocol) 的大模型动作编排能力，使得大模型能够根据对话上下文自主决定和编排动作：

1. **原子动作与情感表达**：针对本地端侧轻量模型（如 Qwen2.5 0.5B/1.5B），系统提供扁平化的 `pose_head`、`pose_body`、`pose_antenna` 及 `play_emotion` (如开心、害羞、思考等) 等原子工具，模型可以在对话时调用简单的物理动作和情感预设动作。
2. **复杂连续动作编排**：针对拥有更强 Function Calling 能力的大模型，系统提供了 `execute_choreography` 工具。大模型可以输出包含多步姿态、相对运动、预设舞蹈等任意组合的嵌套 JSON 序列，由本地引擎负责平滑插值和安全限位执行。

**⚠️ 注意：** 复杂编排功能（即多步骤、多轴协调、嵌套时间线的动作生成）由于强依赖模型的复杂结构化输出能力，**当前仅支持接入 OpenAI 格式接口的大模型（如 OpenAI API、DeepSeek 官方 API 或兼容 OpenAI 格式的服务）**。若使用本地轻量模型（如通过 llama.cpp 部署的小参数模型），由于模型能力限制极易生成格式错乱的 JSON，系统默认推荐使用原子动作模式。
...
用户: "开心地跟我打招呼"
机器人: "你好呀！"  → (LLM 调用 play_emotion 动作)
